...
### 3.2 語音預處理機制與品質稽核特別說明

為克服臺灣南島語言各語料來源在採集與典藏過程中的高度異質性（跨 MP3/AAC/WAV 多種封裝、偽副檔名、前後冗餘靜音、語速異常及長篇錄音缺乏切分等），本計畫導入標準化音訊預處理管線（涵蓋 Stage 0 長音訊切分與 Stage 2 標準化稽核），兼顧聲學高保真度與嚴密之資料防外洩機制：

```mermaid
flowchart LR
    Raw["原始音訊<br/>(MP3 / WAV / M4A)"] --> Probe["容錯解碼與切分<br/>(FFmpeg + PyDub)"]
    Probe --> Resample["規格統一<br/>24kHz Mono 16-bit"]
    Resample --> SHA["Pre-VAD 物理指紋<br/>(PCM SHA-256)"]
    SHA --> VAD["Silero VAD 去靜音<br/>(±0.2s Safety Padding)"]
    VAD --> Val["能量與語速檢驗<br/>(RMS + Speaking Rate)"]
    Val --> DNSMOS["客觀品質評估<br/>(Microsoft DNSMOS)"]
    DNSMOS --> CleanWAV["標準化訓練音訊<br/>(16-bit PCM WAV)"]
```

#### 核心預處理步驟與技術規範欄位說明

| 處理階段 / 核心機制 | 採用工具 / 演算法 | 技術規範與工程實作說明 | 語言學考量與專案效益 |
| :--- | :--- | :--- | :--- |
| **多格式解碼與容錯** | FFmpeg / PyDub | 利用 FFmpeg 自動探測（Probe）真實容器與編碼，遇到損毀標頭時啟動副檔名 Secondary Attempt；針對田調長篇故事錄音（如臺大語料庫）依時間戳進行毫秒級精準切片。 | 徹底解決爬取資料中偽副檔名引發的解碼崩潰，並實現長篇語篇之句子級別對齊。 |
| **規格標準化與重採樣** | Torchaudio / PyDub | 全數統一重採樣為 **`24 kHz, 16-bit, Mono PCM WAV`** 格式；雙聲道自動平均（Downmix）。 | 24kHz 可保留高達 12kHz 頻譜細節，能完整收錄南島語言特有之高頻摩擦音（如 `/s/`, `/x/`, `/c/`）與塞音爆破特徵，並兼顧未來 TTS 模型訓練。 |
| **Pre-VAD 物理聲學指紋** | Python `hashlib` (SHA-256) | 於進行任何 VAD 裁切前，直接對記憶體中未壓縮取樣點序列（`raw_data`）抽取 **SHA-256** 雜湊。 | 徹底摒除 ID3 與容器 Header 干擾，將音訊身分與下游超參數完全解耦，作為訓練集與測試集去重及防範資料外洩（Data Contamination）之客觀依據。 |
| **南島語特化 VAD 去靜音** | Silero VAD (ONNX/PyTorch) | 採用深度神經網絡語音偵測模型，並於偵測區間**強制附加首尾各 0.2 秒之安全緩衝邊界（Safety Padding）**。 | 南島語富含**聲門塞音（Glottal Stop `ʼ`）**、輕輔音與字尾清塞音，0.2 秒 Safety Padding 能有效防止弱輔音遭傳統 VAD 誤切。 |
| **聲學能量與異常檢驗** | NumPy / SoundFile | 計算均方根振幅（RMS），設定門檻為 $RMS \ge 0.001$（約 $-60\text{ dBFS}$）。 | 自動過濾錄音中斷、設備故障、斷訊或實質全靜音之無效音檔。 |
| **自適應字元語速檢驗** | 正則表達式規則過濾 | 移除正書法標點與連字符後計算字元語速：一般句子限定於 **1.0 ～ 35.0 字元/秒**；單詞教學篇放寬下限至 **0.5 字元/秒**。 | 排除音訊快放、截斷或文字與音訊題號錯位之異常語料；單詞低門檻則精準包容教學錄音中母音刻意拉長跟讀之情境。 |
| **客觀語音品質評估** | Microsoft DNSMOS (`speechmos`) | 基於神經網絡免參考評估模型（ITU-T P.835 / P.808），評定整體品質（OVRL）、語音失真（SIG）與背景雜訊（BAK）。 | 提供客觀聲學品質量化分數，支援語料品質分級（如 OVRL $\ge 3.0$）與底噪篩選。 |

> **訊號完整性原則（堅持不採破壞性降噪）**：  
> 在音訊預處理全程中，本計畫堅持**不施加激進或破壞性的頻譜濾波降噪演算法**。過度降噪常會抹平原住民族語中極其微弱的聲門爆破與輔音摩擦特徵；取而代之的是透過高品質神經 VAD、嚴格 RMS/語速稽核與客觀 DNSMOS 指標進行把關，確保輸入 ASR 模型學習之語音訊號具備最高程度的聲學真實度與完整性。
