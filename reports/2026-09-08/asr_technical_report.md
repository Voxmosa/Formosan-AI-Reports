...
1. **高訊噪比（Clean）評估表現**：
   - **顯著降低錯誤率**：新模型全體 42 種語言之加權平均字元錯誤率（CER）由「上一期族語 AI 計畫」Baseline 模型的 **2.77%** 大幅降至 **2.12%**（未加權平均由 **2.91%** 降至 **2.19%**），整體相對錯誤率改善達 **23.6%**。
...
2. **低訊噪比（Low SNR, SNR 10dB）抗噪強健性**：
   - 在注入真實世界重度生活背景噪音（MUSAN, SNR = 10 dB）的壓力測試下（**註：本期新模型微調過程中完全未加入任何噪音資料增強**），新模型的加權平均 CER 依然強韌維持在 **4.37%**（未加權 4.50%）。
   - 相較於上一期 Baseline 模型在同等 10dB 雜訊環境下的 **5.19%**（未加權 5.37%），新模型在重度雜訊下的錯誤率**相對降低了 15.8%**（絕對值降低 0.82%），證明新模型即使在未經人為雜訊訓練的條件下，依然具備極佳的內生性抗噪免疫力與聲學泛化實用價值。
...
### 5.1 評估指標說明
本報告之主要核心評估指標為 **CER（字元錯誤率, %）**，反映原住民族語羅馬拼音字母層級的編輯距離（Levenshtein Distance）：
...
- **Baseline 模型**：即**「上一期族語 AI 計畫」所微調產出之 Whisper Large-v2 ASR 模型測試結果**，作為本期模型效能精進與優化之對照基準（Baseline）。
  ...
  > 
  > 這意味著上一期 Baseline 模型在測試時同時享有「外部語料規模更多」以及「測試集先驗記憶（Seen Data）」的雙重優勢；然而，本期新模型在嚴格切分的獨立評估集上，整體辨識成績依然全面顯著超越 Baseline 模型（全體 42 種語言有 40 種顯著進步，加權 CER 相對降幅達 23.6%）。這項對比更加彰顯了本期新模型在高品質訊號重建、專屬語言標籤導流與臺大田野口述語料整合下的真實泛化技術突破。
...
1. **南勢阿美語 (`ami-x-iams`)**：
   - 上一期 Baseline CER 高達 **10.29%**，本期微調後急遽收斂至 **1.32%**（絕對改善達 **-8.97%**，相對錯誤率下降 **87.2%**）。
   - 原因分析：南勢阿美語在子音連綴與部分特有詞彙拼寫上與秀姑巒阿美語存在音韻分歧，上一期 Baseline 模型容易因缺乏語言專屬 Token 而產生代換錯誤；新模型在引入專屬 Token 後，解碼器能準確鎖定南勢阿美語的拼字規則。
2. **茂林魯凱語 (`dru-x-tldr`)**：
   - 上一期 Baseline CER 高達 **14.82%**，本期新模型成功將其壓制至 **8.64%**（絕對改善達 **-6.18%**，相對改善 **41.7%**）。
...
> **說明**：表中 `Baseline CER` 代表上一期族語 AI 計畫 ASR 模型之測試成果；`新模型 CER` 為本期微調之最新表現。

| 語群分類 | 語言代碼 | 中文語言名稱 | 評估句數 | Baseline CER | 新模型 CER | 絕對改善量 | 相對改善率 | 評估判定 |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
...
> 特別需要強調的是，**本期新模型在微調訓練階段完全未採取資料增強（未在訓練語料中混入額外雜音進行訓練）**。在此嚴格條件下，新模型在 SNR 10dB 的加權平均 CER 依然能由上一期 Baseline 模型的 5.19% 壓低至 **4.37%**（相對改善 15.8%）。這項成果證明新模型的抗噪韌性並非來自「預先看過特定合成雜訊」的記憶效應，而是源於扎實的語音訊號重建與 Whisper 多語言聲學先驗的真實泛化。
