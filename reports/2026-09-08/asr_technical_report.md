...
- **Baseline 模型**：即**「上一期族語 AI 計畫」所微調產出之 Whisper Large-v2 ASR 模型測試結果**，作為本期模型效能精進與優化之對照基準（Baseline）。
  > **重要說明（Baseline 訓練語料背景）**：  
  > 值得特別說明的是，**上一期 Baseline 模型在當初訓練的時候，訓練資料裡面實際上已含有目前所有的測試集語料（除了「文化篇」的語料之外）**。換言之，Baseline 模型是在對測試集具備極高語料重疊與記憶優勢的條件下受測；而本期新模型在嚴格切分的獨立評估集上，辨識率依然全面顯著超越 Baseline 模型，更凸顯新模型在多來源語料訓練、聲學重建與專屬語言標籤導流下所展現的真實泛化實力。
...
### 7.5 Baseline 訓練資料重疊背景下的顯著泛化優勢
從資料集歷史背景審視，**上一期 Baseline 模型在當初微調訓練時，其訓練集實際上已經包含了目前測試集中的絕大多數語料（除了「文化篇」教材語料未納入）**。在這種 Baseline 模型享有極高測試集資料重疊（Seen Data）的巨大優勢下，本期新微調模型依然在全體 42 種語言中取得 40 種語言的全面超越，加權平均 CER 降幅達 23.6%（低 SNR 下相對改善 15.8%）。這項對比有力地證明了本期模型的進步絕非依賴記憶，而是真正學會了更具強健性與泛化性的跨族語聲學與語言表徵。
