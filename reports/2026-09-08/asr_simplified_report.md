...
### (2) 9 大語群在平時（Clean）與生活吵雜（SNR 10dB）下的表現總覽
...
## 🚀 5. 給原語會的後續建議與應用方向 (Recommendations for ILRDF)

為將本期高品質 ASR 成果順利落實於公部門與族人日常生活中，團隊提出以下四點務實建議：

1. **推動系統部署時「自動帶入或手動選擇語言標籤」**：  
   未來若將模型串接於手機 App、教學網站或智慧音箱，建議介面提供「選擇語言」選單，或由系統預設帶入該族語代碼（例如阿美族學生登入即自動綁定 `ami-x-pswl`），即可獲得最佳辨識準確度。
2. **展開「文字標記標準化與正規化（Text Normalization）」專案**：  
   目前各語料庫的文字標記多數來自官方教材，品質良好，但偶有不同年份的標點、大小寫或音調符號微幅分歧。建議原語會未來可邀集族語專家研擬統一的文字正規化規則，模型辨識率將可再向上推升 10%～20%。
3. **針對高難度與瀕危語言啟動「重點補強錄音」**：  
...
4. **加速多領域落地應用**：
   ...
   - **文化耆老口述歷史典藏**：加速部落田調口述歷史與傳統歌謠的數位化逐字稿整理。
